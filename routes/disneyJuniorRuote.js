...
const outputDir = path.join(__dirname, '..', 'disneyJunior');
...
router.get('/disneyJunior/start', (req, res) => {
    lastPing = Date.now();
    ...
    ffmpegProcess = spawn(ffmpegPath, [
        ...
        '-i', 'https://fl1.moveonjoy.com/DISNEY_JR/index.m3u8',
        '-map', '0:v', '-map', '0:a',
...
/* ============================
   FRONTEND PING
============================ */
router.get('/disneyJunior/ping', (req, res) => {
  lastPing = Date.now();
...
function clearOutputFolder() {
    ...
    fs.readdir(outputDir, (err, files) => {
        if (err) {
            console.error('Error reading Disney Junior folder:', err);
            return;
        }
        ...
        console.log('🧹 Disney Junior folder cleared.');
    });
}
/* ============================
   MONITOR FRONTEND
