...
const outputDir = path.join(__dirname, '..', 'nickJunior');
...
router.get('/nickJunior/start', (req, res) => {
    lastPing = Date.now();
    ...
    ffmpegProcess = spawn(ffmpegPath, [
        ...
        '-i', 'https://fl1.moveonjoy.com/NICK_JR/index.m3u8',
        '-map', '0:v', '-map', '0:a',
...
/* ============================
   FRONTEND PING
============================ */
router.get('/nickJunior/ping', (req, res) => {
  lastPing = Date.now();
...
function stopStream() {
    ...
    console.log('🛑 Stopping Nick Junior stream...');
...
function clearOutputFolder() {
    ...
    fs.readdir(outputDir, (err, files) => {
        if (err) {
            console.error('Error reading nickJunior folder:', err);
            return;
        }
        ...
        console.log('🧹 nickJunior folder cleared.');
    });
}
/* ============================
   MONITOR FRONTEND
