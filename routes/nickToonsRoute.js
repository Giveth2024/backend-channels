...
const outputDir = path.join(__dirname, '..', 'nickToons');
...
router.get('/nickToons/start', (req, res) => {
    lastPing = Date.now();
    ...
    ffmpegProcess = spawn(ffmpegPath, [
        ...
        '-i', 'http://143.244.60.30/NICKTOONS/index.m3u8',
        '-map', '0:v', '-map', '0:a',
...
/* ============================
   FRONTEND PING
============================ */
router.get('/nickToons/ping', (req, res) => {
  lastPing = Date.now();
...
function stopStream() {
    ...
    console.log('🛑 Stopping NickToons stream...');
...
function clearOutputFolder() {
    ...
    fs.readdir(outputDir, (err, files) => {
        if (err) {
            console.error('Error reading nickToons folder:', err);
            return;
        }
        ...
        console.log('🧹 NickToons folder cleared.');
    });
}
/* ============================
   MONITOR FRONTEND
